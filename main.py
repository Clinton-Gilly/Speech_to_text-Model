...
def listen_print_loop(responses: object, stream: object) -> None:
    """Iterates through server responses and prints them, handling speaker diarization."""
    for response in responses:
        ...
        # Check if there are any words in the alternative
        if result.alternatives[0].words:
            speaker_tag = result.alternatives[0].words[-1].speaker_tag
        else:
            # If no words are available, skip to the next response
            continue
        ...
        # Display interim results, but with a carriage return at the end of the line
        if result.is_final:
            sys.stdout.write(GREEN)
            sys.stdout.write("\033[K")
            sys.stdout.write(f"Speaker {speaker_tag}: {transcript}\n")
...
def main() -> None:
    ...
    config = speech.RecognitionConfig(
        ...
        language_code="en-US",
        max_alternatives=1,
        diarization_config=diarization_config  # Correct way to include diarization
    )
